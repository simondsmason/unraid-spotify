...
expect "# "
send "docker run -d --name spotify-playlist-manager --restart unless-stopped -p 8100:8081 -v /mnt/user/appdata/spotify-playlist-manager:/app/data -v /mnt/user/media/Audio/spotify-backups:/app/backups -e DATA_DIR=/app/data -e BACKUPS_DIR=/app/backups -e WEB_PORT=8081 --log-driver=syslog --log-opt syslog-address=udp://192.168.2.70:513 --log-opt tag=spotify-playlist-manager --label \"net.unraid.docker.icon=https://storage.googleapis.com/pr-newsroom-wp/1/2023/05/Spotify_Primary_Logo_RGB_Green.png\" --label \"net.unraid.docker.managed=dockerman\" --label \"net.unraid.docker.webui=http://\[IP\]:\[PORT:8100\]\" --label \"com.spotify-playlist-manager.environment=production\" spotify-playlist-manager:latest && echo DEPLOY_SUCCESS || echo DEPLOY_FAILED\r"
expect {
    "DEPLOY_SUCCESS" { }
    "DEPLOY_FAILED" {
